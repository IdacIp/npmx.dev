...
// ── Nitro auto-import stubs (must be before importing likes.ts) ─────
...
/** Create a TID rkey from a Date (TID encodes microsecond timestamps) */
...
// ── Integration test with DI (injected fakes, no global stubs) ──────
...
describe('PackageLikesUtils.getLikesEvolution', () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks()
    fakeCache.get.mockResolvedValue(undefined)
  ...
  it('paginates through backlinks and caches the result', async () => {
    ...
    fakeGetBackLinks.mockResolvedValueOnce(makeBacklinksPage([backlink(day1)], 'cursor-page-2'))
    // Page 2 returns no cursor (end)
    fakeGetBackLinks.mockResolvedValueOnce(makeBacklinksPage([backlink(day2)]))

    const utils = new PackageLikesUtils({
      constellation: { getBackLinks: fakeGetBackLinks } as any,
      cache: fakeCache as any,
    })

    const result = await utils.getLikesEvolution('react')

    expect(fakeGetBackLinks).toHaveBeenCalledTimes(2)
    expect(result).toEqual([
  ...
  it('returns cached result without calling constellation', async () => {
    ...
    fakeCache.get.mockResolvedValueOnce(cachedData)
    ...
    expect(fakeGetBackLinks).not.toHaveBeenCalled()
    expect(result).toEqual(cachedData)
  })

  it('returns empty array when no backlinks exist', async () => {
    fakeGetBackLinks.mockResolvedValueOnce(makeBacklinksPage([]))
