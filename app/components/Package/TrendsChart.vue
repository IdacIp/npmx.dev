...
const props = defineProps<{
  ...
  /**
   * When true, shows metric selector (Downloads / Likes).
   * Only used on the compare page.
   */
...
type MetricDef = {
  id: MetricId
  label: string
  fetch: (pkg: string, options: PackageDownloadEvolutionOptions) => Promise<EvolutionData>
}

const METRICS = computed<MetricDef[]>(() => [
  {
    id: 'downloads',
    label: $t('compare.facets.trends.items.downloads'),
    fetch: (pkg, opts) =>
      fetchPackageDownloadEvolution(pkg, props.createdIso ?? null, opts) as Promise<EvolutionData>,
  },
  {
    id: 'likes',
    label: $t('compare.facets.trends.items.likes'),
    fetch: (pkg, opts) => fetchPackageLikesEvolution(pkg, opts) as Promise<EvolutionData>,
  },
])

const selectedMetric = shallowRef<MetricId>('downloads')
...
watch(
  ...
  ([granularityValue]) => {
    ...
    if (
      selectedMetric.value === 'downloads' &&
...
async function loadMetric(metricId: MetricId) {
  ...
  try {
    ...
    // Reuse provided weeklyDownloads for downloads metric when applicable
    if (metricId === 'downloads') {
  ...
  >
    <div class="w-full mb-4 flex flex-col gap-3">
      <div class="flex flex-col sm:flex-row gap-3 sm:gap-2 sm:items-end">
        <div v-if="showMetricSelector" class="flex flex-col gap-1 sm:shrink-0">
          <span class="text-3xs font-mono text-fg-subtle tracking-wide uppercase">
            {{ $t('package.trends.facet') }}
          </span>
          <div
          ...
          >
            <button
              ...
              :key="metric.id"
              type="button"
              class="px-2.5 py-1.75 font-mono text-sm transition-colors"
              :class="
